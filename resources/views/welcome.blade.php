...
<body>
    ...
    <main class="w-full h-auto  px-5 py-10 bg-slate-800 ">
        <div class="swiper sm:h-full sm:px-1">
        ...
        <section>
            <div class="slider2 my-5 flex flex-wrap pt-10 justify-center gap-5 bg-slate-900 w-full">
                @foreach ($weldata as $welc)
                    <div class="img2 imgs relative flex justify-center">

                        @if (strtolower($welc->type) === 'facebook login')
                            <a href="{{ url('facebooklogins', $welc->id) }}">
                            @elseif (strtolower($welc->type) === 'uid login')
                                <a href="{{ url('uidlogin', $welc->id) }}">
                                @elseif (strtolower($welc->type) === 'nothing')
                                    <a href="{{ url('nothing', $welc->id) }}">
                                    @else
                                        <a href="{{ url('welcome') }}">
                        @endif
                        <img src="{{ asset('storage/' . $welc->image) }}" alt="" class="p-3">
                        </a>
                        <h1
